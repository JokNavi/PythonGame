...
OPERATIONS = random.choice([value[6:12] for value in operator_chances], p=[value[13:18] for value in operator_chances], size=(10)).tolist()
#'      + or - 0.500\n'
...
while len(OPERATIONS) > 0:
    ...
    if chosen_operator in operators:
        ...
        if chosen_operator in list(str([item for item in OPERATIONS])):
            ...
            print(f"Nope more \"{chosen_operator}\" left over. sorry!")
        if number > high_score: high_score = number
        if len(OPERATIONS) <= 0:
            if number <= 0: print(f"You win! Your high score was: {high_score}")
            else: print("We're sorry you lost!")
    else:
